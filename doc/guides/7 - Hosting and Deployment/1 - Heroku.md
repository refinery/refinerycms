...
heroku create \$app\_name —stack cedar\
...
<ruby>\
config.assets.initialize\_on\_precompile = true\
</ruby>
...
*initialize\_on\_precompile*\
is false or default; however, you may need to set the
*user\_env\_compile* option\
...
On Amazon S3, create a bucket called “my\_app\_production”. Then add
this to the\
...
<shell>\
heroku config:add S3\_KEY=123key S3\_SECRET=456secret
S3\_BUCKET=my\_app\_production\
</shell>

WARNING. Make sure the config vars you add to Heroku match Refinery’s
environment variables: *S3\_KEY*, *S3\_SECRET*, and *S3\_BUCKET*

If you have created your bucket in a region other than ‘us-east-1’ you
need to\
add S3\_REGION=s3region also.
...
Double check your S3\_ information and make sure that the right buckets
actually\
