...
You can tell Heroku about your S3 keys so that they do not have to be embedded inside
your application:

<shell>
$ heroku config:add S3_BUCKET=bucket S3_KEY=key S3_SECRET=secret S3_REGION=region
</shell>
...
h4. Via Pow Configuration File (for Pow users only)

You will need to create +~/.powconfig+ and fill it with the following:
...
Then you must restart Pow to apply these settings. You can do this by finding the Pow process in OS X's Activity Monitor and clicking 'Quit Process'.
