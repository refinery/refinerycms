...
Think of a Refinery extension as a miniature Rails application running in your +vendor/extensions+ directory. Each extension specifies its own routes in its config directory and has its own views and controllers in its own app directory. In fact, each extension is actually a *Rails E.*  Es can serve up their own images, stylesheets and javascripts by utilizing the asset pipeline, which got introduced in Rails 3.1.  You can define your own models, views, and controllers in an extension and they can take full advantage of all of the functionality that Refinery offers.  

Since an extension is a kind of E, it has the same familiar directory structure.  You'll see folders for your models, views, controllers, helpers, decorators, config, and so forth.  Your code goes in those folders just as it does in a regular Rails application or E.
...
TIP: to see all the options supported by the +refinery:E+ generator just run +rails g refinery:E+.
...
h3. Crudify: The Backbone of Refinery Extensions
