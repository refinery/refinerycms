...
FriendlyId can generate unique slugs within a given scope. For example:
  ...
  class Restaurant < ActiveRecord::Base
    belongs_to :city
    has_friendly_id :name, :use_slug => true, :scope => :city_id
  end
  ...
  seattle = City.find('seattle')
  chicago = City.find('chicago')

  seattle.restaurants.find("joes-diner") # returns 1 record
  chicago.restaurants.find("joes-diner") # returns 1 record
  Restaurant.find("joes-diner") # returns both records


The value for the :scope key in your model follows the same rules as the
:scope option in +validates_uniqueness_of+ macro. That is, it can be 
some database column of the model, or even array of columns.
