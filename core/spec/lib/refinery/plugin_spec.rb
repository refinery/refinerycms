    ...
    describe '#activity_by_class_name' do
      ...
      context 'when the plugin have diferents activities' do
        it 'returns the correct activity' do
          expect(plugin.activity_by_class_name("X::Y::Z").first).to eq(plugin.activity.first)
          expect(plugin.activity_by_class_name("X::Y::ZZ").first).to eq(plugin.activity.last)
        end
      end
    end
